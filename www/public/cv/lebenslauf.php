
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="styles-lebenslauf.css">
    <link rel="stylesheet" href="../styles.css">
    <title>Lebenslauf</title>
</head>
<body>
    
<!-- <section id="lebenslauf"> -->
<?php include dirname(__DIR__) . "/components/header.php"; ?>
    <main id="cv-container">
        <section id="header">Lebenslauf</section>
        <nav id="nav">
            <ul>
                <li class="li" id="exp-li"><a href="#experience">Beruflicher Werdegang</a></li>
                <li class="li"><a href="#years1">Ausbildung</a></li>
                <li class="li"><a href="#languages">Sprachen</a></li>
                <li class="li"><a href="#kenntnisse">ICT Kenntnisse</a></li>
                <li class="li" id="act-li"><a href="#act">Aktivität</a></li>
            </ul>
        </nav>
        <hr>
        <section id="img" class="sections img"><img src="../images/ari.png" alt="ari"></section>
        <section id="personal" class="sections">
            <p>Mostafanejad Ismail
                <br>Buchzelgweg 4, 8053 Zürich
                <br>+41 76 216 65 57
                <br>xusrew.z@gmail.com
                <br>Geburtsdatum: 21.09.1984
                <br>Aufenthaltsbewilligung: B</p>
        </section>
        <section id="experience" class="sections"><h3>Beruflicher Werdegang</h3></section>
        <section id="empty" class="sections"></section>
        <section id="years" class="sections">
            <h4 class="years">2023-2024</h4>
            <h4 class="years">2021-2022</h4>
            <h4 class="years" style="padding-top: 140px;">2016-2017</h4>
            <h4 class="years" style="padding-top: 20px;">2013 und 2015</h4>
            <h4 class="years">2014</h4>
            <h4 class="years" style="padding-top: 20px;">2010-2012</h4>
            <h4 class="years">2008-2010</h4>
            <h4 class="years">2008</h4>
            <h4 class="years">2004-2007</h4>
        </section>
        <section id="jobs" class="sections">
            <p><strong>Webentwicklung</strong></p>
            <p style="padding-top: 10px;"><strong>1st and 2nd Level Support</strong>
                <br>Zürcher Hochschule für angewandte Wissenschaften
                <br>- Bearbeiten und Überwachen von Incidents
                <br>- Bereitstellen, Installieren und Unterhalt von Geräten
                <br>- Instruieren von Benutzenden
                <br>- Veranlassen von Reparaturen</p>
            <p><strong>+Applikations-, Webentwicklung:</strong> link auf
            <a href="https://github.com/Ari-Ario?tab=repositories">Github</a></p>
            <p style="padding-top: 10px;"><strong>Journalist and Filmemacher</strong>, 
                <br> Surprisemagazin, SRF, etc. link auf <a href="https://www.srf.ch/kultur/gesellschaft-religion/gesellschaft-religion-ein-tag-der-sprachen-reportage-vom-kulturplatz-dreh">SRF</a> /
                <a href="https://www.swissfilms.ch/de/person/ismail-mostafanejad/76C265AFDC9544428C2C91D0F558A780">Swiss Films</a></p>

            <p style="padding-top: 10px;"><strong>Englischlehrer</strong> (Freiwillig), ASZ, Zürich und Griechenland</p>
            <p style="padding-top: 10px;"><strong>Übersetzung eines Buches Englisch in Persisch</strong>
            <br>"Natural remedies for health and well-being"</p>
            <p><strong>Freischaffender Journalist und Blogger</strong>, Iran, Tehran</p>
            <p style="padding-top: 10px;"><strong>Englisch- und Mathematiklehrer</strong>, Iran, Tehran</p>
            <p style="padding-top: 10px;"><strong>Journalist Hawlati Ausgabe auf Kurdisch</strong>, Teilzeit </p>
            <p><strong>Dolmetscher</strong> (Teilzeit während Studium)
               <br> Englisch, Kurdisch, Arabisch, Irak</p>
        </section>
        <section id="years1" class="sections">
            <h3 style="padding-left: 20px;">Aus-, Weiterbildung</h3>
            <h4 class="years">2021-2022</h4>
            <h4 class="years">2017-2020</h4>
            <h4 class="years">2016</h4>
            <h4 class="years">2010-2012</h4>
            <h4 class="years">2004-2008</h4>
            <h4 class="years">2005 und 2006</h4>
        </section>
        <section id="bildung" class="sections">
            <p><strong>Wirtschaftsinformatik</strong>, UZH</p>
            <p><strong>HF Diplom in Film</strong>, F und F Schule für Kunst und Design, Zürich</p>
            <p><strong>Zwei Informatik-semester</strong>, ETHZ</p>
            <p><strong>Postgraduate of Media</strong>, Independent Media Center of Kurdistan</p>
            <p><strong>Bachelor Informatik</strong>, Mathematics and Computer Department, Kurdistan</p>
            <p><strong>Median Kurse (Leserbrief)</strong>, Sommersaison, Kurdistan</p>
        </section>
        <section id="languages" class="sections">
            <h3 style="padding-left: 20px;">Sprach Kenntnisse</h3>
            <h4 class="language">Deutsch</h4>
            <h4 class="language">Kurdisch/Persisch</h4>
            <h4 class="language">Englisch</h4>
            <h4 class="language">Arabisch</h4>
            <h4 class="language">Griechisch</h4>

        </section>
        <section id="kenn" class="sections">
            <p class="languagePro">Sehr gut, Goethe Zertifikat C1</p>
            <p class="languagePro">Muttersprache</p>
            <p class="languagePro">Sehr gut, C1</p>
            <p class="languagePro">Gut, B2</p>
            <p class="languagePro">A2</p>
        </section>
        <section id="kenntnisse" class="sections">
            <h3 style="padding-left: 20px;">ICT Kenntnisse</h3>
        </section>
        <section id="adj" class="sections">
            <p><strong>Hard-, Softwaresupport, 1st und 2nd-Level
                <br>Überwachung/Bearbeitung der Incidents
                <br>Applikation-Support, Reparaturen etc.
                <br>Applikationsentwicklung mit Python und C
            <br>Webentwicklung (Fullstack)</strong></p>
        </section>
        <section id="act" class="sections">
            <h3 style="padding-left: 20px;">Aktivität/Interessen</h3>
            <h4 class="LesenSport">Lesen</h4>
            <h4 class="LesenSport">Sport</h4>
        </section>
        <section id="actInfo" class="sections">
            <p>Wissenschaft, Programmieren, und Wirtschaft</p>
            <p>Joggen, Wandern</p>
        </section>
        <section id="ref" class="sections">
            <h3 style="padding-right: 20px;">Referenzen</h3>
        </section>
        <section id="refInfo" class="sections">
            <p>Auf Anfrage</p>
        </section>
        <hr>
        <footer id="footer-lebenslauf">&copy; 2024</footer>
    </main>
<!-- </section> -->
    <script src="../script.js"></script>

<?php include dirname(__DIR__) . "/components/footer.php"; ?>
</body>
</html>